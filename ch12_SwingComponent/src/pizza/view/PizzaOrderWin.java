...
public class PizzaOrderWin extends JFrame {
	...
	private PizzaOrderWin() {
		...
		//Center 구성
		JPanel pnlOfCenter = new JPanel();
		
		
		
		//East 구성
		JPanel pnlOfEast = new JPanel(new GridLayout(0, 1));
		pnlOfEast.setBorder(BorderFactory.createTitledBorder("종류"));
			//콤보, 포테이토, 불고기
		ButtonGroup sizemenu = new ButtonGroup();
		JRadioButton[] btnOfSize = new JRadioButton[3];
		btnOfSize[0] = new JRadioButton("small");
		btnOfSize[1] = new JRadioButton("medium");
		btnOfSize[2] = new JRadioButton("larget");
		for (JRadioButton btn : btnOfSize) {
			menu.add(btn);
			pnlOfEast.add(btn);
		}
		this.add(pnlOfEast, BorderLayout.EAST);
