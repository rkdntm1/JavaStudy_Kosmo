...
public class Board {
	...
	public Board() {
		...
		for (int row = 0; row < ROOT; row++) {
			for (int col = 0; col < ROOT; col++) {
				cells[row][col] = new Cell(row, col); // 9칸을 새롭게 답자
			}
		}

		makeVerticalLines(); // 세로
		makeHorizontalLines(); // 가로
		makeDiagnalineLines(); // 대각선
		makeSlanDiagnalineLines(); // 기울어진대각선
	}
	 ...
	 * 이는 Controller에서 빈칸이 있을 때만 play()를 요청하기 때문
	 */
	public Cell play() {
	 ...
	 * 대각선 만들기 00 11 22
	 */
	private void makeDiagnalineLines() {
	 ...
	 */
	private void makeSlanDiagnalineLines() {
